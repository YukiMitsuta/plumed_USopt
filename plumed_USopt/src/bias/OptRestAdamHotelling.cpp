...
//+PLUMEDOC BIAS OPTIMIZERESTRAINTHOTELLING
/*
...
class OptimizeRestraintHotelling : public Bias {
  ...
  std::vector<double> meandamp;
  std::vector<long double> hotelling_max;
  ...
  std::vector<std::vector<double>> cv_vec;
  std::vector<long int> cv_hodelling_bool;
  double v_cv;
  double v_kappa;
  ...
  double tot_work;
public:
  explicit OptimizeRestraintHotelling(const ActionOptions&);
  void calculate() override;
  static void registerKeywords( Keywords& keys );
};

PLUMED_REGISTER_ACTION(OptimizeRestraintHotelling,"OPTIMIZERESTRAINTHOTELLING")

void OptimizeRestraintHotelling::registerKeywords( Keywords& keys ) {
  ...
  */
  ...
  keys.add("compulsory","HOTELLINGMAX"," the maximum of Maharanobis distance");
  ...
  keys.addFlag("NOOPTSIGMA",false," not optimize sigma");
  keys.addFlag("MINUSSIGMAGRAD",false," gradient to smaller sigma");
...
OptimizeRestraintHotelling::OptimizeRestraintHotelling(const ActionOptions&ao):
  PLUMED_BIAS_INIT(ao),
  ...
  minussigmagrad(false)
{
  ...
  // Try to read HOTELLINGMAX
  parseVector("HOTELLINGMAX",hotelling_max);
  // Try to read offdiagonal
  parseFlag("OFFDIAGONAL",offdiagonal);
  // Try to read nooptsigma
  parseFlag("NOOPTSIGMA",nooptsigma);
  // Try to read ignore_cv
  parseVector("IGNORECV",ignore_cv);
  // Try to read minussigmagrad
  parseFlag("MINUSSIGMAGRAD",minussigmagrad);
...
void OptimizeRestraintHotelling::calculate() {
  double ene=0.0;
  double totf2=0.0;
  ...
  std::vector<int> cv_hodelling_bool(stride[0]);
  ...
  if(now==0) {
    ...
    for(int j=0;j<stride[0]; j++) {
      cv_hodelling_bool[j]=1;
    };
    MatrixXd cmat(narg,narg);
    ...
    for(int j=0;j<stride[0]; j++) {
      ...
      if (exp_decay_ave[0] == 0 ){
        for(i=0; i<narg; ++i) {meandamp[i] = 0.0;};
        for (int k=0;k<stride[0]; k++) {
          if (cv_hodelling_bool[k]) {
            for(i=0; i<narg; ++i) {
              meandamp[i] += cv_vec[k][i];
            };
            Nc += 1.0;
          };
        };
        for(i=0; i<narg; ++i) {mean[i] /=Nc;};
      } else {
        double Nmeandamp = Nmean;
        for (int k=0;k<stride[0]; k++) {
          if (cv_hodelling_bool[k]) {
            Nmeandamp += 1.0;
            Nc += 1.0;
            for(i=0; i<narg; ++i) {
              if (Nmeandamp < exp_decay_ave[0]){
                meandamp[i] = meandamp[i]*(Nmeandamp-1.0)/Nmeandamp+cv_vec[k][i]/Nmeandamp;
              } else {
                meandamp[i] = meandamp[i]*(1.0 - 1.0/exp_decay_ave[0])+cv_vec[k][i]/exp_decay_ave[0];
              };
            };
          };
        };
      }
      for(unsigned m=0; m<narg; ++m) {
        for(unsigned n=0; n<narg; ++n) {
          cmat(m,n) = 0.0;
        };
      };
      for (int k=0;k<stride[0]; k++) {
        if (cv_hodelling_bool[k]) {
          for(unsigned m=0; m<narg; ++m) {
            for(unsigned n=0; n<narg; ++n) {
              cmat(m,n) += (cv_vec[k][m]-meandamp[m])*(cv_vec[k][n]-meandamp[n]);
            };
          };
        };
      };
      ...
      MatrixXd cmatinv = cmat.inverse();
      bool endhodellingQ = true;
      for (int k=0;k<stride[0]; k++) {
        if (cv_hodelling_bool[k]) {
          double d = 0.0;
          for(unsigned m=0; m<narg; ++m) {
            for(unsigned n=0; n<narg; ++n) {
              d += (cv_vec[k][m]-meandamp[m])*cmatinv(m,n)*(cv_vec[k][n]-meandamp[n]);
            };
          };
          if (hotelling_max[0]*hotelling_max[0]<d) {
            cv_hodelling_bool[k] = 0;
            endhodellingQ = false;
          };
        };
      };
      if( endhodellingQ ) break;
    };
    mean = meandamp;
    log.printf("Nc = %f\n",Nc);
    if (exp_decay_ave[0] != 0 ){Nmean+=Nc;};
    aa_before=at;
    for(i=0; i<narg; ++i) {
      mean[i] += aa_before[i];
      getPntrToComponent(getPntrToArgument(i)->getName()+"_mean")->set(mean[i]);
    }
    SelfAdjointEigenSolver<MatrixXd> ES(cmat);
    MatrixXd U = ES.eigenvectors();
    MatrixXd Uinv = U.inverse();
    VectorXd e = ES.eigenvalues();
    MatrixXd L = MatrixXd::Zero(narg,narg);
    for(i=0; i<narg; ++i) {
      L(i,i) = targetsigma[i]*targetsigma[i]-e(i);
      if (!minussigmagrad){
        if (L(i,i)>0.0) {
          L(i,i) = 0.0;
        };
      ...
      if (offdiagonal) {
        ...
        for(i=0; i<narg; ++i) {
          ...
          for(j=0; j<narg; ++j) {
            ...
            at[i] -= stepsize_cv[0]*mean_target_j*kk_before_offd[i][j];
/*
            if (i==j){
